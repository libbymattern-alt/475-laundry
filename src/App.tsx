...
<div style={{ fontSize: 19, fontWeight: 500, color: “var(–color-text-primary)”, marginBottom: 4 }}>
{machine.label}
</div>
<div style={{ fontSize: 13, color: “var(–color-text-secondary)”, marginBottom: 20 }}>
Pick your unit and how long you need the machine.
</div>

```
    {/* Unit picker */}
    <div style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-tertiary)", textTransform: "uppercase" as const, letterSpacing: 1.5, marginBottom: 10 }}>
      Your apt
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 20 }}>
      {UNITS.map(u => (
        <button key={u} onClick={() => setUnit(u)} style={{
          ...
          fontSize: 15, fontWeight: unit === u ? 600 : 400,
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6, marginBottom: 10 }}>
      {DURATION_PRESETS.map(p => (
        <button key={p.value} onClick={() => { setDurationMins(p.value); setShowCustom(false); }} style={{
          ...
          fontSize: 13, fontWeight: !showCustom && durationMins === p.value ? 600 : 400,
    ...
    <button
      onClick={() => { localStorage.setItem("laundry-unit", unit); onConfirm(unit, effectiveDuration * 60 * 1000); }}
      disabled={effectiveDuration <= 0}
      style={{
        width: "100%", background: effectiveDuration > 0 ? accent : "var(--color-background-secondary)",
        border: "none", color: effectiveDuration > 0 ? "#fff" : "var(--color-text-tertiary)",
    ...
    <button onClick={onCancel} style={{
      ...
      color: "var(--color-text-tertiary)", padding: "10px",
      cursor: "pointer", fontSize: 14, borderRadius: 12,
    }}>Cancel</button>
  </div>
</div>
```
...
<div style={{ fontSize: 19, fontWeight: 500, color: “var(–color-text-primary)”, marginBottom: 6 }}>
Clear {machine.label}?
</div>
<div style={{ fontSize: 13, color: “var(–color-text-secondary)”, marginBottom: 24 }}>
{session?.unit ? `Unit ${session.unit}'s` : “This”} session will be cleared and the machine marked available.
</div>
<button onClick={onConfirm} style={{
width: “100%”, background: “#E57373”, border: “none”, color: “#fff”,
...
```
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      {ALL_MACHINES.map(m => {
        ...
        return (
          <div
            key={m.id}
            onClick={() => inUse ? onClear(m) : onStart(m)}
            style={{
          ...
          >
              ...
              {inUse ? (
                <>
                  <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 1 }}>
                    ...
                    {session.cycleNum ? ` · #${session.cycleNum}` : ""}
                  </div>
            ...
            {inUse && progress !== null && (
              <div style={{ position: "absolute" as const, bottom: 0, left: 0, right: 0, height: 3, borderRadius: "0 0 8px 8px", overflow: "hidden", background: "transparent" }}>
  ...
  {activeSessions.length === 0 && (
    <div style={{ textAlign: "center" as const, padding: "40px 24px", color: "var(--color-text-tertiary)", fontSize: 18 }}>
...
Just tap a machine to claim it and set a timer. That’s it.
</div>
</div>
{steps.map((step, i) => (
<div key={i} style={{
background: i === 0 ? “#E1F5EE” : “var(–color-background-primary)”,
border: `0.5px solid ${i === 0 ? "#9FE1CB" : "var(--color-border-tertiary)"}`,
...
background: “var(–color-background-primary)”, border: “0.5px solid #9FE1CB”,
borderRadius: “var(–border-radius-lg)”, padding: “14px 18px”,
fontSize: 13, color: “#0F6E56”, maxWidth: 320, zIndex: 1000,
...
if (!st || isNaN(st) || st < 1_000_000_000_000) return;
if (now - st > s.durationMs + 60000) { // 1 min grace period
...
await set(ref(db, `sessions/${startTarget.id}`), {
unit,
startTime: Date.now(),
durationMs,
cycleNum,
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Fraunces:wght@300;600&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; } select, input, textarea { appearance: none; }`}</style>
...
```
  ...
  {clearTarget && (
    <ClearModal
      machine={clearTarget}
      session={sessions[clearTarget.id]}
  ...
  <div style={{ borderBottom: "0.5px solid var(--color-border-tertiary)", padding: "20px 24px", background: "var(--color-background-primary)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
    <div>
      ...
      <div style={{ fontSize: 11, color: connected ? "#1D9E75" : "#BA7517", marginTop: 2 }}>
  ...
  <div style={{ display: "flex", borderBottom: "0.5px solid var(--color-border-tertiary)", padding: "0 16px", background: "var(--color-background-primary)", overflowX: "auto" as const }}>
    {tabs.map(tab => (
      <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
        ...
        letterSpacing: 1, textTransform: "uppercase" as const, fontWeight: 400,
      }}>{tab.label}</button>
    ))}
  </div>
