...
import { getDatabase, ref, onValue, set, runTransaction, push } from “firebase/database”;
...
// Format how long a machine has been running, e.g. “23 min” or “1h 5m”
function formatDuration(startTime: any, now: number): string | null {
...
style={{ width: “100%”, background: “var(–color-background-secondary)”, border: “0.5px solid var(–color-border-tertiary)”, color: “var(–color-text-primary)”, borderRadius: 10, padding: “10px 12px”, fontSize: 14, outline: “none”, resize: “none” as const, fontFamily: “inherit”, marginBottom: 10 }}
/>
...
{messages.length === 0 && (
<div style={{ textAlign: “center” as const, padding: “32px 24px”, color: “var(–color-text-tertiary)”, fontSize: 14 }}>No messages yet. Be the first to post!</div>
)}
{messages.map(msg => (
...
<div style={{ fontSize: 14, color: “var(–color-text-secondary)”, lineHeight: 1.6 }}>The laundry room runs itself. Just check the Status tab to see what’s available.</div>
</div>
{steps.map((step, i) => (
...
<div style={{ fontSize: 14, color: “var(–color-text-secondary)” }}>When a machine starts and nobody has claimed it, it’ll show up here.</div>
</div>
);
}
...
const duration = formatDuration(s.startTime, now);
return (
...
{s.cycleNum ? `cycle ${s.cycleNum} · ` : “”}
{duration ? `running ${duration}` : “running”}
</div>
</div>
...
const remaining = getRemaining(session, now);
const duration = formatDuration(session?.startTime, now);
...
{done ? “↑ Ready!” : remaining !== null ? formatTime(remaining) + “ left” : duration ? `running ${duration}` : “running”}
</div>
</>
            ...
            <div key={machineId} style={{ background: "var(--color-background-primary)", border: `0.5px solid ${done ? accentBorder : "var(--color-border-tertiary)"}`, borderRadius: "var(--border-radius-md)", padding: "14px 16px", marginBottom: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: progress !== null ? 10 : 0 }}>
                ...
                <div style={{ fontSize: 13, color: accent, fontWeight: 500 }}>
                  {done ? "Done!" : remaining !== null ? formatTime(remaining) : duration ?? "running"}
                </div>
              </div>
              {progress !== null && (
                <>
              ...
              {progress === null && (
                <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 6 }}>
                  sensor detected · {duration ? `running ${duration}` : "auto"}
                </div>
              )}
            </div>
          );
